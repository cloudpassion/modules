...
class DbBreakException(BaseException): pass


class MyEventMessageDatabase(
    MyChatDatabase,
    MyTelegramParseMedia,
    MyTelegramReactions,
):
    ...
    async def database_message(
            ...
            delete=False, check=False,
            _media_messages=None,
    ):
        try:
            return await self._database_message(
                message=message, delete=delete, check=check,
