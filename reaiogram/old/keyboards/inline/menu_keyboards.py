...
async def categories_keyboard(dispatcher):
    ...
    categories = await dispatcher.orm.get_categories()
    for category in categories:
        # Чекаем в базе сколько товаров существует под данной категорией
        number_of_items = await dispatcher.orm.count_items(category.category_code)
...
async def items_keyboard(category, subcategory, dispatcher):
    CURRENT_LEVEL = 2
    ...
    items = await dispatcher.orm.get_items(category, subcategory)
    for item in items:
        # Сформируем текст, который будет на кнопке
