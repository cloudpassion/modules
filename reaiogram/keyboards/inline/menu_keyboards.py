...
async def categories_keyboard(dispatcher):
    ...
    categories = await dispatcher.sql.get_categories()
    for category in categories:
        # Чекаем в базе сколько товаров существует под данной категорией
        number_of_items = await dispatcher.sql.count_items(category.category_code)
...
async def subcategories_keyboard(category, dispatcher):
    ...
    subcategories = await dispatcher.sql.get_subcategories(category)
    for subcategory in subcategories:
        # Чекаем в базе сколько товаров существует под данной подкатегорией
        number_of_items = await dispatcher.sql.count_items(
...
async def items_keyboard(category, subcategory, dispatcher):
    ...
    items = await dispatcher.sql.get_items(category, subcategory)
    for item in items:
        # Сформируем текст, который будет на кнопке
