...
class TorrentGrabVersion5_1(
    DefaultTorrent,
    AbstractMergedTelegram
):

    async def grab_torrent_from_telegram_version5_1(
            self,
            out_dir,
    ):

        return await self.grab_torrent_from_telegram_version5(
            out_dir=out_dir,
        )

        # to bad code
        # need to make structure metadat and data in one file
    #     create_dir(out_dir)
    ...
    #
    #     dj_docs = [
    #         d for d in DjangoTgDocument.objects.all(
    #         ) if f'{d.file_name}.txt' in dji_docs_names
    #     ]
    #
    #     txt_messages = [
    #         m for m in DjangoTgMessage.objects.all(
    #         ) if m.document in dj_docs
    #     ]
    #
    #     logger.info(f'{len(dj_docs)=}, {dj_docs[:10]}')
    #
    #     txt_messages = []
    #     sorted(txt_messages, key=lambda txt_msg: txt_msg.id)
    #
    ...
    #             continue
    #
    #         if txt_msg.document.file_name.split('.')[0] not in dji_docs:
    ...
    #         await self._merge_some_version5_1(
    ...
    #
    # async def _merge_some_version5_1(
    #         self,
    #         txt_bytes,
    #         dj_pieces,
    #         out_dir,
